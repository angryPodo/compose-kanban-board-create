...
- 태그는 최대 5개까지 입력할 수 있으며, 각 태그는 1자 이상 5자 이하의 형식이어야 합니다.
- 사용자가 입력을 시도한 이후부터 유효성 에러 메시지가 노출되도록 하여 사용자 경험을 개선합니다.

### 폼 상태 관리 및 컴포넌트 분리 (진행 중)
- UI 컴포넌트가 지고 있는 무거운 상태 관리 및 검증 책임을 분리하여, 순수하게 화면을 그리는 역할에 집중하도록 구조를 개선합니다.
- 입력된 폼 데이터를 안전하게 상위로 전달하기 위한 데이터 구조체를 활용합니다.
